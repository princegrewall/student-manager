...
const Library = () => {
    ...
    try {
      ...
      if (isTeacher && user?.id) {
        ...
        filteredData = data.filter((book) => {
          const bookOwnerId = typeof book.addedBy === "object" ? book.addedBy?._id || book.addedBy?.id : book.addedBy

          const isOwner = bookOwnerId === user.id

          if (!isOwner) {
            console.log(
              `Excluding book "${book.title}" because owner ID ${bookOwnerId} doesn't match current user ID ${user.id}`,
            )
          } else {
            console.log(
              `Including book "${book.title}" because owner ID ${bookOwnerId} matches current user ID ${user.id}`,
            )
          }
        ...
        console.log(
          `After filtering: ${filteredData.length} books remain (removed ${data.length - filteredData.length})`,
        )
      }
      ...
      if (filteredData && filteredData.length > 0) {
        filteredData.forEach((book) => {
          ...
          let semesterNum = Number.parseInt(String(book.semester)) || 1
    ...
    try {
      ...
      if (user?.id) {
        ...
        filteredData = data.filter((book) => {
          ...
          if (!isOwner) {
            console.warn(
              `Filtering out book "${book.title}" because owner ID ${bookOwnerId} doesn't match current user ID ${user.id}`,
            )
          }
        ...
        console.log(
          `After client-side filtering: ${filteredData.length} books (removed ${data.length - filteredData.length})`,
        )
      }
  ...
  const getFilteredBooksBySemester = (semester: number) => {
    ...
    if (booksBySemester[semester]) {
      return booksBySemester[semester].filter(
        (book) =>
          book.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
          book.author.toLowerCase().includes(searchTerm.toLowerCase()),
      )
    ...
    return booksToFilter.filter((book) => {
      const bookSemester = Number.parseInt(String(book.semester)) || 1
      return (
  ...
  const handleDeleteBook = async () => {
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          setBooksBySemester((prev) => {
            ...
            Object.keys(newState).forEach((semesterKey) => {
              const semester = Number.parseInt(semesterKey)
          ...
          if (apiErr.permissionDenied || (apiErr.message && apiErr.message.includes("Not authorized"))) {
        ...
        if (err.permissionDenied || (err.message && err.message.includes("Permission denied"))) {
          setError("You cannot delete books uploaded by other teachers. Only your own uploads can be deleted.")
        }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const semester = Number.parseInt(String(newBook.semester)) || 1
    ...
    return (
      <div className="min-h-screen flex flex-col bg-gradient-to-br from-teal-950 to-slate-900 text-white">
        ...
        <main className="flex-grow container mx-auto py-8 px-4 flex justify-center items-center">
          <RefreshCw className="h-8 w-8 animate-spin mr-2 text-teal-400" />
          ...
          <div className="space-y-8">
            <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
              <div>
                ...
                <p className="text-teal-300">Browse, download, and manage educational resources</p>
              </div>
            ...
            <div className="relative">
              <Search className="absolute top-2.5 left-3 h-5 w-5 text-teal-300" />
              <Input
                type="text"
                placeholder="Search by title or author..."
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                className="pl-10 bg-slate-800/50 border-slate-700 text-white focus:border-teal-500 focus:ring-1 focus:ring-teal-500"
              />
            </div>
            ...
            {isTeacher && (
              <div className="w-full">
                {/* Teacher header - simplified to just the semester view */}
                <div className="bg-slate-800/50 backdrop-blur-sm w-full p-4 mb-6 rounded-md border border-slate-700">
                  <div className="flex justify-between items-center">
                    <div>
                      <h2 className="text-xl font-bold text-white">My Books by Semester</h2>
                      <p className="text-teal-300 text-sm mt-1">
                        View your uploaded books organized by semester.{" "}
                        <span className="text-amber-300">Note: As a teacher, you can only see your own uploads.</span>
                      </p>
                    </div>
                    <Button onClick={fetchMyLibraryData} className="bg-teal-600 hover:bg-teal-700 text-white" size="sm">
                ...
                {loading ? (
                  <div className="flex justify-center py-20">
                    <div className="flex items-center space-x-2">
                      <RefreshCw className="animate-spin h-5 w-5 text-teal-400" />
                      <span className="text-teal-300">Loading all books...</span>
                    </div>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 gap-4">
                    {semesters.map((semester) => {
                      ...
                      const semesterBooks = teacherSemBooks.filter(
                        (book) =>
                          book.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
                          book.author.toLowerCase().includes(searchTerm.toLowerCase()),
                      )
                      ...
                      return (
                        <div
                          key={semester}
                          className="bg-slate-800/40 rounded-lg border border-slate-700 overflow-hidden backdrop-blur-sm hover:border-teal-700 transition-all duration-300"
                        >
                          <button
                            ...
                            <div className="flex items-center space-x-4">
                              ...
                              <div className="text-left">
                                ...
                                <p className="text-teal-300 text-sm">
                                  {semesterBooks.length} resource{semesterBooks.length !== 1 ? "s" : ""}
                                </p>
                              </div>
                            </div>
                            {isExpanded ? (
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-4">
                              {semesterBooks.length > 0 ? (
                                semesterBooks.map((book) => (
                                  <Card
                                    key={book._id}
                                    className="overflow-hidden bg-slate-800/70 border-slate-700 hover:border-teal-600 transition-all duration-300 shadow-lg"
                                  >
                                    <CardHeader className="p-4 pb-2 space-y-1 bg-gradient-to-r from-teal-900/30 to-slate-800/30">
                                    ...
                                    <CardContent className="p-4 pt-2 space-y-4">
                                      ...
                                      <div className="flex items-center justify-between">
                                        <Button
                                          ...
                                          className="bg-teal-900/50 text-teal-300 hover:bg-teal-800/70 border border-teal-700/50"
                                        >
                                        ...
                                        {canEditLibrary && (isBookOwner(book) || isCoordinator) && (
                                          <div className="flex gap-2">
                                            <Button
                                              ...
                                              className="h-8 w-8 bg-slate-800 border-slate-700 hover:bg-slate-700 hover:border-teal-600"
                                            >
                                            ...
                                            <Button
                                              ...
                                              className="h-8 w-8 bg-slate-800 border-slate-700 hover:border-red-700 hover:bg-red-950/30"
                                            >
                                ...
                                <div className="col-span-full text-center p-8">
                                  <BookOpen className="h-12 w-12 mx-auto mb-3 text-teal-800" />
                    ...
                    {myBooks.length === 0 && (
                      <div className="text-center py-16 bg-slate-800/40 rounded-xl border border-slate-700 mt-4 backdrop-blur-sm">
                        <BookOpen className="h-16 w-16 mx-auto mb-4 text-teal-700" />
                        <h3 className="text-xl font-medium text-white mb-2">No books uploaded yet</h3>
                        <p className="text-teal-300 mb-6 max-w-md mx-auto">
              ...
              <>
                {loading ? (
                  <div className="flex justify-center py-20">
                    <div className="flex items-center space-x-2">
                      <RefreshCw className="animate-spin h-5 w-5 text-teal-400" />
                      <span className="text-teal-300">Loading library resources...</span>
                    </div>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 gap-4">
                      ...
                      return (
                        ...
                        >
                          ...
                          {isExpanded && (
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-4">
                              {semesterBooks.length > 0 ? (
                                // Show books if semester has items
                                semesterBooks.map((book) => (
                                  <Card
                                    key={book._id}
                                    className="overflow-hidden bg-slate-800/70 border-slate-700 hover:border-teal-600 transition-all duration-300 shadow-lg"
                                  >
                                    <CardHeader className="p-4 pb-2 space-y-1 bg-gradient-to-r from-teal-900/30 to-slate-800/30">
                                      ...
                                      <CardDescription className="text-sm text-teal-300">
                                        By {book.author}
                                        {typeof book.addedBy === "object" && book.addedBy?.name && (
          ...
          <DialogContent className="bg-slate-900 text-white border-slate-700 max-w-2xl">
            ...
            <div className="py-4">
              <div className="flex items-center space-x-2 mb-4">
                ...
                <p className="text-teal-100 font-medium">{selectedBook.author}</p>
              </div>

              <p className="text-teal-100 mb-6">{selectedBook.description}</p>
          ...
          <DialogHeader>
            ...
            <DialogDescription className="text-teal-300">
              {editMode
                ...
                : "Add a new book to the library. Required fields are marked with *"}
            </DialogDescription>
          </DialogHeader>

          <div className="grid gap-4 py-4">
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="title" className="text-right text-teal-300">
                Title *
              </Label>
              <Input
                id="title"
                ...
                className="col-span-3 bg-slate-800 border-slate-700 text-white focus:border-teal-500 focus:ring-teal-500"
                required
              />
            </div>

            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="author" className="text-right text-teal-300">
                Author *
              </Label>
              <Input
                id="author"
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="semester" className="text-right text-teal-300">
                Semester *
              </Label>
              <select
                id="semester"
                value={newBook.semester}
                onChange={(e) => handleSemesterChange(Number.parseInt(e.target.value))}
                className="col-span-3 bg-slate-800 border-slate-700 rounded p-2 text-white focus:border-teal-500 focus:ring-teal-500"
                required
              >
                {semesters.map((sem) => (
            ...
            <div className="grid grid-cols-4 items-start gap-4">
              <Label htmlFor="description" className="text-right pt-2 text-teal-300">
                Description
              </Label>
              <Textarea
                id="description"
                ...
                className="col-span-3 bg-slate-800 border-slate-700 text-white min-h-24 focus:border-teal-500 focus:ring-teal-500"
              />
            </div>

            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="link" className="text-right text-teal-300">
                External Link
              </Label>
              <Input
                id="link"
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="file" className="text-right text-teal-300">
                Upload File
              </Label>
              <div className="col-span-3">
                <Input
                  ...
                  className="bg-slate-800 border-slate-700 text-white focus:border-teal-500 focus:ring-teal-500"
                />
                {selectedFile && (
            ...
            {error && (
              <div className="bg-red-900/30 border border-red-500 p-3 rounded-lg mt-2">
                <p className="text-red-300 text-sm">{error}</p>
              </div>
            )}
          </div>
