...
import { Trophy, Code, Music, Monitor, Mic2, Users, Calendar, Plus, MapPin, UserPlus, UserPlus2, Layers, Trash2, UserMinus, User } from "lucide-react";
import { clubsAPI, eventsAPI, adminAPI, authAPI } from '@/lib/api';
...
const ClubDetails = () => {
      ...
      try {
        ...
        if (hasPermission('crud:clubs') || hasPermission('view:members')) {
          try {
            ...
            const defaultsForType = type && defaultSubclubs[type as keyof typeof defaultSubclubs];
            if (defaultsForType) {
              setSubclubs(defaultsForType.map(sc => ({
                ...sc,
                members: [],
                createdAt: new Date().toISOString()
              })));
  ...
  const handleCreateSubclub = async () => {
    ...
    try {
      ...
      setSubclubs(prevSubclubs => [...prevSubclubs, {
        ...createdSubclub,
        members: [],
        createdAt: new Date().toISOString()
      }]);
      ...
      try {
        ...
        if (err.message && err.message.includes('not found')) {
          if (err.message.includes('Student not found')) {
            setStudentError(`Error: The email "${studentEmail}" is not registered in the system. Only registered students can be added to clubs.`);
          } else {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#1A1F2C] text-white">
      ...
      <main className="flex-grow container mx-auto px-4 py-8">
        <div className="flex justify-between items-center mb-8">
          <div className="flex items-center">
            <ClubIcon className="h-8 w-8 mr-3 text-purple-400" />
            <h1 className="text-3xl font-bold">{title}</h1>
          </div>
          
          <div className="flex space-x-3">
            {/* Delete Club Button - Only for coordinators */}
            {/* Removed the Delete Club button as requested */}
        ...
        <Tabs defaultValue="subclubs" className="w-full">
          <TabsList className="mb-6 bg-[#222222]">
            <TabsTrigger value="subclubs" className="data-[state=active]:bg-purple-600">Subclubs</TabsTrigger>
          ...
          <TabsContent value="subclubs" className="space-y-4">
            ...
            >
              <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
                      <CardTitle className="text-xl font-bold text-white">{subclub.name}</CardTitle>
                      <ClubIcon className="h-5 w-5 text-purple-400" />
              </CardHeader>
              <CardContent>
                      <p className="text-sm text-gray-400 mb-2">{subclub.description}</p>
                      <p className="text-sm text-blue-400">{subclub.members?.length || 0} members</p>
                    </CardContent>
                  </Card>
                ))
              ) : (
            ...
            {selectedSubclub && (
              <Dialog open={!!selectedSubclub} onOpenChange={(open) => !open && setSelectedSubclub(null)}>
                <DialogContent className="bg-[#222222] text-white border-[#333333] max-w-2xl">
                  <DialogHeader>
                  ...
                  <div className="space-y-4 py-4">
                    ...
                    {canAddStudents && (
                      <div className="mt-4">
                        ...
                        <div className="space-y-4 bg-[#333333] p-4 rounded-md">
                          <div className="space-y-2">
                    ...
                    {loadingSubclub ? (
                      ...
                      <div className="space-y-2 max-h-60 overflow-y-auto pr-2">
                        {subclubMembers.map((member) => (
                          <div key={member._id} className="flex items-center justify-between p-2 hover:bg-gray-700 rounded">
                            <div className="flex items-center">
          ...
          {/* Events Tab - Keep existing events section */}
          <TabsContent value="events" className="space-y-4">
            ...
            {loading ? (
              ...
              <div className="space-y-4">
                {events.map((event) => (
                  <Card key={event._id} className="bg-[#2A2A2A] border-[#444444]">
                    <CardContent className="pt-4">
                      <div className="flex justify-between items-start">
                        <div>
                          <h3 className="text-lg font-bold">{event.title}</h3>
                          <p className="text-gray-400 text-sm mt-1 mb-2">{event.description}</p>
      ...
      {eventToDelete && (
        <Dialog open={!!eventToDelete} onOpenChange={(open) => !open && setEventToDelete(null)}>
          <DialogContent className="bg-[#222222] text-white border-[#333333]">
            ...
            <div className="py-4">
              <p className="text-gray-300 mb-4">Are you sure you want to delete this event? This action cannot be undone.</p>
              {error && (
          ...
          <DialogContent className="bg-[#222222] text-white border-[#333333]">
            ...
            <div className="py-4">
              ...
              <p className="text-gray-300 mb-4">
                Are you sure you want to delete the subclub "{subclubToDelete.name}"? 
                This will remove all members from the subclub.
              </p>
              {error && (
                <p className="text-red-500 text-sm mt-2 mb-2">{error}</p>
              )}
      ...
      {confirmDeleteClub && (
        <Dialog open={confirmDeleteClub} onOpenChange={setConfirmDeleteClub}>
          <DialogContent className="bg-[#222222] text-white border-[#333333]">
            <DialogHeader>
              <DialogTitle>Confirm Club Deletion</DialogTitle>
            </DialogHeader>
            <div className="py-4">
              <p className="text-red-500 font-semibold mb-2">Warning: This action cannot be undone!</p>
              <p className="text-gray-300 mb-4">
                Deleting this club will permanently remove the club, all its subclubs, 
                and all associated events. All members will be removed from the club.
              </p>
              {error && (
                <p className="text-red-500 text-sm mt-2 mb-2">{error}</p>
              )}
              <div className="flex justify-end space-x-2">
                ...
                <Button 
                  ...
                  className="bg-red-600 hover:bg-red-700"
                >
                  {loading ? 'Deleting...' : 'Delete Club Permanently'}
                </Button>
              </div>
            </div>
          </DialogContent>
          ...
          <DialogContent className="bg-[#222222] text-white border-[#333333]">
            ...
            <div className="py-4">
              <p className="text-gray-300">
                Are you sure you want to remove <span className="font-semibold">{studentToRemove.name}</span> from the subclub?
              </p>
            </div>
            <DialogFooter>
              <Button 
