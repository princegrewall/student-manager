...
import { 
    ...
    ChevronDown, Calendar, Clock, Info
} from "lucide-react";
import { 
    Collapsible,
    CollapsibleContent,
        ...
        const fetchData = async () => {
            try {
                ...
                for (const subject of subjectsData) {
                    ...
                    const percentage = percentageResponse.percentage || 
                        (percentageResponse.data ? percentageResponse.data.percentage : 0);
    ...
    return (
        <div className="min-h-screen flex flex-col bg-[#1A1F2C] text-white">
            ...
            <main className="flex-grow container mx-auto px-4 py-8">
                ...
                <Card className="bg-[#222222] border-[#333333] mb-8">
                    <CardHeader>
                ...
                {loading ? (
                    ...
                    <p className="text-gray-400">No attendance records added yet.</p>
                ) : (
                    <div className="space-y-4">
                        {subjectsWithAttendance.map((item) => (
                            <Card key={item.subject._id} className="bg-[#222222] border-[#333333]">
                                            ...
                                            <Button 
                                                ...
                                                className={`${
                                                    item.lastMarked === 'Present'
                                                        ...
                                                        : 'text-green-500 border-green-500 hover:bg-green-900/20'
                                                }`}
                                            >
                                            ...
                                            <Button 
                                                ...
                                                className={`${
                                                    item.lastMarked === 'Absent'
                                                        ...
                                                        : 'text-red-500 border-red-500 hover:bg-red-900/20'
                                                }`}
                                            >
                                    ...
                                    <div className="grid grid-cols-3 gap-4 mb-4 bg-[#1A1A1A] p-3 rounded-md">
                                        <div className="text-center p-2">
                                            <p className="text-gray-400 text-xs uppercase">Total Classes</p>
                                            <p className="text-xl font-bold mt-1">{item.records.length}</p>
                                            <p className="text-xs text-gray-500 mt-1">All time</p>
                                        </div>
                                        <div className="text-center p-2">
                                            <p className="text-gray-400 text-xs uppercase">Present</p>
                                            <p className="text-xl font-bold mt-1 text-green-500">
                                                {item.records.filter(r => r.status === 'Present').length}
                                            </p>
                                            <p className="text-xs text-green-700 mt-1">
                                                {Math.round((item.records.filter(r => r.status === 'Present').length / 
                                                    Math.max(item.records.length, 1)) * 100)}% of total
                                            </p>
                                        </div>
                                        <div className="text-center p-2">
                                            <p className="text-gray-400 text-xs uppercase">Absent</p>
                                            <p className="text-xl font-bold mt-1 text-red-500">
                                                {item.records.filter(r => r.status === 'Absent').length}
                                            </p>
                                            <p className="text-xs text-red-700 mt-1">
                                                {Math.round((item.records.filter(r => r.status === 'Absent').length / 
                                                    Math.max(item.records.length, 1)) * 100)}% of total
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="bg-[#1A1A1A] p-3 rounded-md mb-4">
                                        <h4 className="text-sm font-medium mb-2">This Week's Progress</h4>
                                                ...
                                                return (
                                                    <div key={index} className="flex flex-col items-center">
                                                        ...
                                                        <div 
                                                            ...
                                                            ${dayRecord?.status === 'Present' ? 'bg-green-900/50 text-green-400' :
                                                              dayRecord?.status === 'Absent' ? 'bg-red-900/50 text-red-400' : 
                                                              isPast ? 'bg-yellow-900/30 text-yellow-500' : 
                                    ...
                                    <div className="bg-[#1A1A1A] p-3 rounded-md mb-4">
                                        <div className="flex justify-between items-center">
                                            <div>
                                                ...
                                                {item.records.length > 0 ? (
                                                    <p className="text-lg font-bold mt-1">
                                                        {calculateCurrentStreak(item.records)} day{calculateCurrentStreak(item.records) !== 1 ? 's' : ''}
                                                    </p>
                                                ) : (
                                                    <p className="text-lg font-bold mt-1">0 days</p>
                                                )}
                                            </div>
                                            <div>
                                            ...
                                            <div>
                                                ...
                                                <p className="text-lg font-bold mt-1">
                                                    {calculateMonthAttendance(item.records).present}/{calculateMonthAttendance(item.records).total} days
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="bg-[#1A1A1A] p-3 rounded-md mb-4">
                                        <h4 className="text-sm font-medium mb-2">Forecast & Goals</h4>
                                        <div className="grid grid-cols-2 gap-4">
                                            <div className="bg-[#222222] rounded-md p-3">
                                                <p className="text-gray-400 text-xs uppercase mb-1">Projected Attendance</p>
                                                <div className="flex justify-between items-center">
                                                    <p className="text-lg font-medium">
                                                        {calculateProjectedAttendance(item.records)}%
                                                    </p>
                                            ...
                                            <div className="bg-[#222222] rounded-md p-3">
                                                <p className="text-gray-400 text-xs uppercase mb-1">To Reach 75% Minimum</p>
                                                {calculateAttendanceNeeded(item.records) > 0 ? (
                                                    <>
                                                        ...
                                                        <p className="text-xs text-gray-500 mt-2">
                                                            To achieve minimum required attendance
                                                        </p>
                                                    </>
                                                ) : (
                                                    <>
                                                        ...
                                                        <p className="text-xs text-gray-500 mt-2">
                                                            You've met the minimum attendance requirement
                                                        </p>
                                                    </>
                                                )}
                                            </div>
                                        ...
                                        <div className="flex justify-between items-center mt-2">
                                            <p className={`text-sm ${item.percentage >= 75 ? 'text-green-500' : 'text-red-500'}`}>
                                                {item.percentage >= 75 
                                                    ? 'Good Standing' 
                                                    : item.percentage >= 65
                                            ...
                                            {item.percentage < 75 && (
                                                <p className="text-xs text-yellow-500">
                                                    Need {Math.ceil((75 * item.records.length - 
                                                        (item.records.filter(r => r.status === 'Present').length * 100)) / 25)} 
                                    ...
                                    {item.records.length > 0 && (
                                        <Collapsible className="mt-4 border-t border-[#333333] pt-4">
                                            ...
                                            <CollapsibleContent className="mt-2">
                                                ...
                                                <div className="mb-3 p-3 bg-[#1A1A1A] rounded-md">
                                                        ...
                                                        {generateLast30DaysAttendance(item.records).map((day, index) => (
                                                            <div 
                                                                ...
                                                                className={`w-6 h-6 rounded-sm flex items-center justify-center text-xs font-medium ${
                                                                    day.status === 'Present' ? 'bg-green-900/50 text-green-400' :
                                                                    day.status === 'Absent' ? 'bg-red-900/50 text-red-400' : 
                                                                    'bg-[#2A2A2A] text-gray-500'
                                                                }`}
                                                                title={day.date ? `${new Date(day.date).toLocaleDateString()}: ${day.status}` : 'No record'}
                                                            >
                                                    ...
                                                    <div className="grid grid-cols-3 gap-2">
                                                        <div>
                                                            ...
                                                            <p className="text-sm font-medium">
                                                                {calculatePeriodAttendance(item.records, 7).present}/{calculatePeriodAttendance(item.records, 7).total} days
                                                            </p>
                                                        </div>
                                                        <div>
                                                            <p className="text-xs text-gray-400">Last 30 Days</p>
                                                            <p className="text-sm font-medium">
                                                                {calculatePeriodAttendance(item.records, 30).present}/{calculatePeriodAttendance(item.records, 30).total} days
                                                            </p>
                                                        </div>
                                                        <div>
                                                        ...
                                                        .map(record => (
                                                            <div 
                                                                ...
                                                                className={`flex justify-between items-center py-2 px-3 rounded-md ${
                                                                    record.status === 'Present' ? 'bg-green-900/20' : 'bg-red-900/20'
                                                                }`}
                                                            >
                                                                <div className="flex items-center">
                                                ...
                                                {item.records.length > 0 && (
                                                    <div className="flex items-center mt-2 p-2 bg-blue-900/20 rounded-md">
